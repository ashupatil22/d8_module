...
class LatestNodeTitleBlock extends BlockBase implements ContainerFactoryPluginInterface {
	...
	public function __construct(array $configuration, $plugin_id, $plugin_definition, Connection $database, AccountProxy $Account) {
		...
		$this->account = $Account->getAccount ( 'current_user' );
	}
	...
	public function build() {
		$build = [ ];
		// kint ( $this->account->getEmail () );
		$name = $this->fetchData ();
		$build ['latest_node_title_block'] ['#markup'] = implode ( "|", $name ['titles'] ) . '|' . $this->account->getEmail ();
	...
	public function fetchData() {
		$titles = [ ];
		$query = $this->database->select ( 'node_field_data', 'nfd' )->fields ( 'nfd', [
				'nid',
				'title'
		] )->orderBy ( 'created', 'DESC' )->range ( 0, 3 );
